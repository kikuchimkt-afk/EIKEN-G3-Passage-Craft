...
const MarkdownRenderer = ({ text, translations = [], onSentenceClick, highlightedSentence }) => {
  ...
  const splitTextIntoSentences = (text) => {
    ...
    // Split by sentence-ending punctuation (and optional quote/asterisk) followed by space or newline
    return text.split(/(?<=[.!?]["”]?(?:\*)?)\s+/);
  };
  ...
  const handleSyntaxClick = (content, type) => {
    ...
    const findInSentences = (sentences) => {
      ...
      if (type === 'blockquote') {
        ...
        // Normalize function: lower case, remove asterisks, trim
        const normalize = (s) => s.toLowerCase().replace(/\*/g, '').trim();
        const normContent = normalize(cleanContent);

        // 1. Try exact/include match (Case-insensitive, Asterisk-insensitive)
        foundIndex = sentences.findIndex(t => {
          const normSentence = normalize(t.en);
          // Check if clean content is in sentence OR sentence is in clean content
          return normSentence.includes(normContent) || normContent.includes(normSentence);
        });
        ...
        if (foundIndex === -1) {
          ...
          if (chunks.length > 0) {
            for (const chunk of chunks) {
              ...
              foundIndex = sentences.findIndex(t => {
                const normSentence = normalize(t.en);
                return normSentence.includes(normChunk);
              });
    ...
    if (targetSentence && targetTab) {
      ...
      setTimeout(() => {
        ...
        let targetSpan = null;
        const normalize = (s) => s.toLowerCase().replace(/\*/g, '').trim();
        const normTarget = normalize(targetSentence);

        for (let span of spans) {
          const spanText = normalize(span.textContent);

          // Allow loose matching for finding the DOM element
          if ((normTarget.includes(spanText) || spanText.includes(normTarget)) && spanText.length > 5) {
            targetSpan = span;
            break;
          }
          // Fallback: Check first 20 chars
          if (spanText.includes(normTarget.substring(0, 20))) {
            targetSpan = span;
            break;
          }
        }
